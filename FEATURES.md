...
| Atomic + idempotent JSON save (`SaveJSON`)      | FULLY_FUNCTIONAL           | `autoconfigure.go:123`; creates parent dirs, skips write when content unchanged (no inode change, race-tested), crash-durable via go-atomic-write; returns `changed bool`; concurrent modification surfaces as `*ConfigError` wrapping `atomicwrite.ErrConcurrentModification` |
