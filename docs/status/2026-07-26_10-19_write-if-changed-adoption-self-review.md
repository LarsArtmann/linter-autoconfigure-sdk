...
| ~~2~~ | ~~Expose `changed` bool from `SaveJSON`~~ done — `(changed bool, *ConfigError)` shipped in v0.1.0 (TODO_LIST T16)                                                            | ~~`WriteIfChanged` returns `(changed bool, err)`; `SaveJSON` discards the bool. Callers can't tell whether the file was actually written. A `SaveJSONIfChanged(path, v) (bool, *ConfigError)` variant or a return-type change would fix this — but it's a breaking signature change.~~                                                   |
| ~~3~~ | ~~Test `SaveJSON`'s `WriteIfChanged` error path~~ done (CHANGELOG `[0.1.0]`: read-only-dir wraps `fs.ErrPermission`)                                                           | ~~The `if _, err := atomicwrite.WriteIfChanged(...); err != nil` branch is uncovered (SaveJSON at 88.9%). Needs filesystem mocking or a write-to-read-only-dir fixture.~~                                                        |
...
6. ~~**`SaveJSON` loses information by discarding `changed`.** The whole point of `WriteIfChanged` is to tell the caller whether the write happened. `SaveJSON` throws that away. The design should expose it.~~ done — `(changed bool, *ConfigError)` since v0.1.0 (T16)

7. ~~**Error granularity regressed.** The old `SaveJSON` had distinct `Op` values for each failure stage. The new one collapses everything from `WriteIfChanged` into `OpWrite`. An `OpVerify` or wrapping `ErrConcurrentModification` with a distinct `Op` would preserve the granularity.~~ resolved — `errors.Is(err, ErrConcurrentModification)` documented in godoc; `OpVerify` never warranted
...
| ~~2~~ | ~~Rewrite idempotency test to be filesystem-independent (inode change count or content-hash, not mtime)~~ done — `os.SameFile` (CHANGELOG `[0.1.0]`)          | ~~Current test is flaky~~    |
| ~~3~~ | ~~Add test for `SaveJSON`'s `WriteIfChanged` error path (write to read-only dir)~~ done (CHANGELOG `[0.1.0]`)                                                                                                 | ~~Uncovered branch~~            |
...
| #      | Task                                                                                                                                                              | Impact                                     |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| ~~16~~ | ~~Expose `changed` from `SaveJSON` — add `SaveJSONIfChanged(path, v) (bool, *ConfigError)` or change return type~~ done — return type changed in v0.1.0 (the variant name was rejected as a lying name)                                                    | ~~Caller loses key info~~                      |
| ~~17~~ | ~~Add `OpVerify` or surface `ErrConcurrentModification` distinctly in `SaveJSON` errors~~ resolved — documented `errors.Is` path; Op split unnecessary                                                                             | ~~Error granularity regressed~~                |
...
| ~~22~~ | ~~Add integration test: concurrent `SaveJSON` calls to same path~~ done (CHANGELOG `[0.1.0]`: barrier writers surface the wrapped sentinel)                                                                    | ~~Race-safety of the full stack~~              |
...
| #  | Task                                                                                       | Impact                                 |
| -- | ------------------------------------------------------------------------------------------ | -------------------------------------- |
| ~~33~~ | ~~Build `ProviderFromSpec(spec)` adapter (README promises it)~~ done — shipped v0.1.0 (before any consumer)                                | ~~Core SDK value prop~~                    |
| ~~34~~ | ~~Wire a real consumer: `golangci-lint-auto-configure` against new SDK~~ done — v0.2.0 era → v0.10.0                      | ~~Validates SDK design~~                       |
| ~~35~~ | ~~Wire a real consumer: `oxlint-auto-configure` against new SDK~~ done — v0.2.0 era → v0.9.1 (fully on the SDK)                                 | ~~Validates SDK design~~                       |
...
| ~~46~~ | ~~Add `gosec` scan for the new file I/O paths~~ done — gosec runs via the generated `.golangci.yml` (G301 fixed in `[0.2.0]`)                                             | ~~Security audit~~                             |
| ~~47~~ | ~~Verify `buildflow --fix --fail-on-findings` passes on SDK (strict gate)~~ dispositioned — strict carries 9 accepted advisories (documented baseline in AGENTS.md, 2026-09-23); plain gate fully green            | ~~CI readiness~~                               |
...
16 more rows resolved above: the `changed` bool shipped in v0.1.0 (c2/f16,
with `SaveJSONIfChanged` rejected as a lying name), the error-path and
concurrency tests landed with it, both consumers are wired (f33/f34/f35,
v0.2.0 era), gosec runs via the generated `.golangci.yml`, and the strict-gate
question is dispositioned as the documented 9-advisory baseline. Still open:
c4/f24/f25 (hallucinated-commit history rewrite — owner decision), the
go-atomic-write-side website/test items (#7/#8/#10/#11/#13/#14/#15/#28/#29),
f18/f19 (idempotency demo in `ExampleSaveJSON`), f21 (retry design), f32
(migration guide), f36-f45 (future features/fuzz/stress), f49/f50.
