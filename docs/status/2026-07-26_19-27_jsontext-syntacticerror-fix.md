...
1. ~~**`erraudit` — 1 finding.** `autoconfigure.go:197` `FindingsFromIssues` returns generic
   ...
   would have addressed this if it were not explicitly out of scope.~~ done — decided:
   plain `error` kept, wrap carries the tool name, erraudit zeroed at `d674922`
   (CHANGELOG `[0.2.0]`)
...
9. ~~Did not add a regression test specifically for `*jsontext.SyntacticError` round-trip
   ...
   both, so the assertion could be richer.~~ done — `TestConfigError_{Is,As}Delegates
   toWrappedCause` (CHANGELOG `[0.1.0]`)
...
1. ~~Fix the unchecked `os.RemoveAll` in `example_test.go:17` and `:33` — trivial, 2-line.~~ done —
   `_ =` discards, v1.10.0 session 2026-09-10 (CHANGELOG `[0.1.0]`)
...
4. ~~Add `internal/` directory or document why the flat layout is intentional, to
   satisfy `go-structure-linter`.~~ done — flat-layout rationale documented in
   `.golangci.yml` header + AGENTS.md (T11); step skipped with reason
5. ~~Move `example_test.go` into an `examples/` directory (or add a stub), to satisfy
   the same linter.~~ done — decided: godoc examples live in `example_test.go` at
   root (documented; renders on pkg.go.dev)
6. ~~Add a `.golangci.yml` at the repo root, even a minimal one, to silence the
   missing-config finding.~~ done at `4e86443`; regenerated by the auto-configurer
   (`[0.2.0]`, ~106 linters)
...
7. ~~Decide on the error-type strategy for `FindingsFromIssues`: introduce a typed
   ...
   and silence the `erraudit` finding per-line.~~ done — plain `error` kept with full
   context in the wrap (see (e)1)
8. ~~Add a regression test covering `(*ConfigError).As(any) bool` delegation to
   `errors.As(e.Err, target)` — currently only `errors.AsType` is asserted.~~ done
   (CHANGELOG `[0.1.0]`)
9. ~~Add a one-line "Requires `GOEXPERIMENT=jsonv2`" note to the package-level doc
   comment in `autoconfigure.go:1-17`.~~ done at v0.1.0 — later moot: the requirement
   retired with the go 1.27 floor (v0.3.0)
...
16. ~~Build out a first real consumer (`golangci-lint-auto-configure` or
    ...
    `FindingFromIssue`, and `ProviderSpec.HasRepair` together.~~ done — both consumers
    live since the v0.2.0 era
...
18. ~~Add a `pareto-planning` pass over the SDK to identify the next 20% of work that
    delivers 80% of consumer value.~~ done — the 2026-09-22 SDK Absorption
    Masterplan is exactly that (duplication kill D1-D7 → v0.3.1-v0.6.0)
19. ~~Publish the SDK on pkg.go.dev once a tagged release exists (currently no tags on
    master).~~ done — v0.1.0 2026-09-10; every version renders (verified 2026-09-23)
20. ~~Set up a CI workflow that runs `buildflow` on PRs (currently the project uses
    `buildflow` locally; CI integration is undocumented).~~ done at `4e86443` —
    portable subset (full buildflow job = T21, blocked)
...
22. ~~Replace the remaining generic-`error` returns (`FindingsFromIssue`,
    ...
    `*ConfigError`.~~ decided — plain `error` kept (see (e)1); findings-carrying
    conversions are semantically not config I/O
23. ~~Add a `go-finding` integration test that exercises the full finding-emission
    ...
    done — `toolsdk.Register` acceptance test (v0.1.0) + 18 bootstrap contract tests
    (v0.5.0)
24. ~~Add `examples/` directory with a runnable example that wires the SDK into a
    BuildFlow detector + repairer pair, end-to-end.~~ done — `ExampleProviderFromSpec`
    + `ExampleBootstrapProviderFromSpec` + the README's spec→Register walkthrough
...
31. ~~Run `architecture-review` on the SDK to confirm the package boundary is correct
    for the planned consumer split.~~ done — the 2026-07-26 HTML review (frozen) + the
    2026-09-22 masterplan's gap analysis confirm the boundary
32. ~~Run `data-model-review` on `ConfigIssue`, `ConfigError`, `Op`, `ProviderSpec` —
    types are small but warrant a first-principles pass.~~ done — the archived
    "make-architecture-and-data-model-superb" plan executed exactly this
    (2026-07-26_06-05)
...
36. ~~Add `gosec` or equivalent to the buildflow pipeline.~~ done — gosec runs via the
    generated `.golangci.yml` (G301 fixed, `[0.2.0]`)
...
40. ~~Update README.md with a "Quick start" section showing the typical 5-step consumer
    setup (per the `website-launch` skill pattern).~~ done — README
    Installation/Usage sections + compile-guarded snippets
...
41. ~~Make the SDK the canonical foundation for ALL linter auto-configurers
    ...
    achieved for the Go/JS pair — two consumers live; every "reinvented identically"
    row in the README table is SDK-owned
42. ~~Document the SDK's compatibility policy: Go version, jsonv2 build tag, dependency
    ...
    stability risk for downstream).~~ done — README states the Go 1.27+ floor and peer
    deps; AGENTS records the always-latest policy; per-release Dependencies
    changelog entries carry each bump
...
50. ~~Add a `ProviderSpec` validation function that catches obvious misconfiguration
    (e.g. `Repair` returns a non-nil string error with no clear contract).~~ done —
    exported sentinels in `ProviderFromSpec` (v0.2.0, `c7049d4`) + bootstrap
    sentinels (v0.5.0)
