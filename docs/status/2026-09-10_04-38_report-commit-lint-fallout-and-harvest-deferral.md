...
| -- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------ | ------------- | ---------- |
| ~~1~~  | ~~Decide markdown-lint policy repo-wide: configure MD013/wide-table handling once, or formally accept the wallpaper (then stop counting it)~~ done at `0cc2c2e` — MD013 at 120 with tables/code exempt; strict lane now runs at a documented 9-advisory baseline | ~~High~~   | ~~S~~      | ~~Decision~~      | NEW — d1   |
...
| ~~4~~  | ~~Run HARVEST: route report #1's 10 new Tier-1 items + this report's #1–3 into TODO_LIST/ROADMAP~~ done — 2026-09-11 defaulted ruling; re-applied by the 2026-09-23 masterplan harvest (T31-T43) | ~~High~~   | ~~S~~      | ~~Documentation~~ | NEW — b1   |
...
| ~~6~~  | ~~Ecosystem map into both AGENTS.mds (go-finding / go-atomic-write / go-linter-sdk / this SDK / BuildFlow)~~ done for THIS repo (AGENTS.md "What this is", 2026-09-23); the go-linter-sdk mirror (#2 of report #1) remains unverified | ~~High~~   | ~~S~~      | ~~Documentation~~ | #1 rep.    |
| ~~7~~  | ~~Add `ProviderSpec.Detector()` adapter (Analyze + FindingsFromIssues ≡ `finding.Detector`)~~ superseded — `ProviderFromSpec` (v0.1.0) already wraps Analyze as the Detector; bootstrap Detect via `BootstrapProviderFromSpec` (v0.5.0) | ~~High~~   | ~~M~~      | ~~Feature~~       | #3 rep.    |
| ~~8~~  | ~~ConfigIssue-vs-direct-findings ADR (converter philosophy vs Repair contract)~~ done — boundary decision documented in AGENTS.md "What this is" (categories/tags stay app-side) | ~~High~~   | ~~S~~      | ~~Decision~~      | #6 rep.    |
| ~~9~~  | ~~Verify external fetch: `go get` from a clean throwaway module (repo is public; fetch never proven)~~ done 2026-09-10; scripted `bcd9302`       | ~~Critical~~ | ~~S~~      | ~~Quality~~       | T1         |
| ~~10~~ | ~~Trigger + verify pkg.go.dev listing (godoc, examples, license)~~ done — v0.1.0 verified 2026-09-11; all versions v0.1.0–v0.6.0 fetched 2026-09-23 | ~~High~~   | ~~S~~      | ~~Quality~~       | T2         |
| ~~11~~ | ~~Add CI workflow (`buildflow --fix --fail-on-findings`, `GOEXPERIMENT=jsonv2`) — ends manual-only health claims like a4/b3~~ done at `4e86443` — portable subset (buildflow job = TODO_LIST T21, blocked) | ~~High~~   | ~~M~~      | ~~Quality~~       | T3         |
| ~~12~~ | ~~Cut `v0.1.0` + GitHub Release (gated on question g2)~~ done 2026-09-10 (`ae510be`); ladder through v0.6.0                                                                                                                                             | ~~High~~   | ~~S~~      | ~~Release~~       | T4         |
| ~~13~~ | ~~Fix the 2 live errcheck findings (`os.RemoveAll` in `example_test.go:17/:33`) — re-confirmed today~~ done — v1.10.0 session 2026-09-10 (`_ =` discards)                                                                  | ~~Medium~~ | ~~S~~      | ~~Bug~~           | T5         |
...
| ~~15~~ | ~~First consumer migration: wire `golangci-lint-auto-configure` onto the SDK end-to-end — the milestone that decides the SDK's right to exist~~ done — v0.2.0-era (both consumers); deep migration through v0.10.0/v0.9.1 (2026-09-23) | ~~Critical~~ | ~~L~~      | ~~Feature~~       | R          |
| ~~16~~ | ~~`ErrNoRepair` sentinel test + doc example (exported, untested)~~ superseded — documented deprecated alias (`d674922`); removal at v1 = TODO_LIST T20                                                                       | ~~Medium~~ | ~~S~~      | ~~Quality~~       | T6         |
| ~~17~~ | ~~`(*ConfigError).As`/`.Is` delegation regression tests (only `AsType` asserted)~~ done (CHANGELOG `[0.1.0]`)                                               | ~~Medium~~ | ~~S~~      | ~~Quality~~       | T7         |
| ~~18~~ | ~~Filesystem-independent idempotency test (replace 20ms-sleep mtime comparison)~~ done — `os.SameFile` (CHANGELOG `[0.1.0]`)                               | ~~Medium~~ | ~~S~~      | ~~Quality~~       | T8         |
| ~~19~~ | ~~`SaveJSON` `changed`-bool decision (zero consumers = free now)~~ done — `(changed bool, *ConfigError)` shipped in v0.1.0 (T16)                           | ~~Medium~~ | ~~S~~      | ~~Decision~~      | T16        |
| 20 | Rename/repurpose decision: `project-autofix-sdk` broadening — formally accept or reject                                                                   | High     | S      | Decision      | R          |
| ~~21~~ | ~~SECURITY.md + branch protection on public `master`~~ done — `4e86443` + API 2026-09-10 (required checks at the v0.1.0 release)                          | ~~Medium~~ | ~~S~~      | ~~Quality~~       | T12/T13    |
| ~~22~~ | ~~README public sales-page audit from a clean machine (gated on question g3)~~ done 2026-09-10 (g3 resolved); kept compile-true by `readme_snippets_test.go` (`bcd9302`) | ~~Medium~~ | ~~M~~      | ~~Documentation~~ | T18        |
| ~~23~~ | ~~GOEXPERIMENT=jsonv2 note in package doc (`autoconfigure.go:1-17`)~~ done at v0.1.0 — later moot: requirement retired with the go 1.27 floor (v0.3.0)   | ~~Medium~~ | ~~S~~      | ~~Documentation~~ | T10        |
| 24 | Fuzz + property-based Save→Load round-trip tests                                                                                                          | Medium   | M      | Quality       | R          |
| ~~25~~ | ~~`ProviderFromSpec` → `toolsdk.Spec` BuildFlow adapter (only after consumer exists, per ROADMAP)~~ done — shipped in v0.1.0, before any consumer           | ~~High~~   | ~~M~~      | ~~Feature~~       | R          |
...
19 rows resolved inline. #1 (MD013) landed at `0cc2c2e`; the T-cluster shipped
across v0.1.0-v0.2.0; the ecosystem-map/ADR items closed via AGENTS.md on
2026-09-23. Still open, now tracked: #2 (dprint reconciliation — routed to
TODO_LIST T44 this pass; `dprint.json` still runs in no pipeline step) and #3
(`--staged-only` pre-commit gate — never adopted repo-wide). #5 (slim config),
#14 (go-linter-sdk source re-validation), #20 (rename/repurpose — ROADMAP),
#24 (fuzz/property — ROADMAP) remain open where they live.
