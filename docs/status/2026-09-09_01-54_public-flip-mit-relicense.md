...
| ~~b4~~ | ~~Public-readiness of the repo surface~~ | ~~README exists and is decent (9 KB); MIT badge now truthful~~                  | ~~README content never audited for external-consumer needs (~~GOEXPERIMENT=jsonv2 requirement lives only in AGENTS.md~~ _[claim was wrong: README.md:35-36 has documented it since `1818692`]_; install instructions unverified)~~ done — audited 2026-09-10 (f12); install verified end-to-end and scripted (`bcd9302`) | ~~None~~                                   | ~~S~~                                  |
...
| ~~3~~ | ~~Verify module fetchable from a clean external module: `go get github.com/larsartmann/linter-autoconfigure-sdk`~~ done 2026-09-10, scripted `bcd9302` | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~4~~ | ~~Trigger pkg.go.dev listing (proxy fetch) and verify the rendered package page (godoc, examples, license)~~ done — v0.1.0 verified 2026-09-11; every version fetched 2026-09-23 | ~~High~~ | ~~S~~ | ~~Quality~~ |
| ~~5~~  | ~~Add CHANGELOG entry: MIT relicensing + public flip (c1)~~ done at `e46c225`                                                                                                                                                                         | ~~High~~   | ~~S~~  | ~~Documentation~~ |
| ~~6~~ | ~~Add CI: `.github/workflows/ci.yml` running `buildflow --fix --fail-on-findings` on push/PR (c4)~~ done at `4e86443` — portable subset (buildflow job = T21, blocked) | ~~High~~ | ~~M~~ | ~~Quality~~ |
| ~~7~~ | ~~Decide + cut first tag `v0.1.0` so pkg.go.dev shows a real version (c3, g2)~~ done 2026-09-10 (`ae510be`); ladder through v0.6.0 | ~~High~~ | ~~S~~ | ~~Release~~ |
| ~~8~~ | ~~Create GitHub Release with notes for the tag~~ done — every tag has one (plain release marked Latest; prerelease+Latest rejected by the API with 422) | ~~Medium~~ | ~~S~~ | ~~Release~~ |
...
| ~~12~~ | ~~Audit README as a public sales page: install snippet, API example, comparison to hand-writing config~~ done 2026-09-10; kept compile-true by `readme_snippets_test.go` (`bcd9302`) | ~~Medium~~ | ~~M~~ | ~~Documentation~~ |
| ~~13~~ | ~~Update AGENTS.md: public status, MIT, tag/CI state (c5)~~ done at `e46c225`                                                                                                                                                                         | ~~Medium~~ | ~~S~~  | ~~Documentation~~ |
| ~~14~~ | ~~Add SECURITY.md with a security contact~~ done at `4e86443` | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~15~~ | ~~Enable branch protection on master (no direct pushes to a public default branch)~~ done 2026-09-10 (force-push/deletion blocked; required checks since v0.1.0) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
| ~~16~~ | ~~Review `docs/planning/license-domain-fit-analysis.md` for public exposure: it proposes renaming/repurposing the SDK (strategy). Move insight to ROADMAP, relocate file~~ done — routed: the rename/repurpose proposal lives as a ROADMAP candidate; the analysis file stays (fate rides Q1) | ~~Medium~~ | ~~S~~ | ~~Cleanup~~ |
...
| ~~18~~ | ~~Sweep all tracked `docs/status/*.md` + `docs/planning/*.md` for internal project names (licenseforge, BuildFlow internals) and redact or accept~~ done — ACCEPT (T17; CHANGELOG `[0.1.0]`) | ~~Medium~~ | ~~S~~ | ~~Cleanup~~ |
| ~~19~~ | ~~Add GitHub topics: `go`, `linter`, `golangci-lint`, `oxlint`, `sdk`, `config`~~ done 2026-09-10 (`4e86443` era) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~20~~ | ~~Review repo description; consider adding install command~~ done 2026-09-10 | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~21~~ | ~~dependabot.yml (or renovate) for the public repo~~ done at `4e86443` (weekly, grouped) | ~~Medium~~ | ~~S~~ | ~~Quality~~ |
...
| ~~23~~ | ~~Verify package-level godoc on `autoconfigure.go` reads well as the pkg.go.dev landing text~~ done — refreshed 2026-09-23 (diff+discovery+bootstrap bullets); renders verified | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~24~~ | ~~Verify `example_test.go` Examples render on pkg.go.dev (they are the public API demo)~~ done — example set verified on every version page (2026-09-23) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
...
| ~~30~~ | ~~Add issue + PR templates~~ done at `4e86443` | ~~Low~~ | ~~S~~ | ~~Documentation~~ |
| ~~31~~ | ~~Add CODEOWNERS~~ done at `4e86443` | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~32~~ | ~~Set GitHub social preview image~~ done — uploaded + og:image byte-verified live 2026-09-11; guarded in CI since (522c660) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~33~~ | ~~Document in README which tools consume this SDK (currently both private — the "why does this exist" story is invisible)~~ done — README Consumers section; both repos public (ROADMAP Q3 resolved) | ~~Medium~~ | ~~S~~ | ~~Documentation~~ |
| ~~34~~ | ~~Decide whether consumer tools eventually go public (g3); if yes, plan their own go-public checklists (e1)~~ done — both consumers public (verified 2026-09-10; ROADMAP Q3) | ~~Medium~~ | ~~M~~ | ~~Feature~~ |
...
| ~~43~~ | ~~Re-check proxy/pkg.go.dev propagation 24h after items 3-4 (propagation can lag)~~ done — re-verified repeatedly since (2026-09-23: all versions render) | ~~Low~~ | ~~S~~ | ~~Quality~~ |
| ~~44~~ | ~~Add coverage reporting (buildflow already writes `reports/coverage.out`) as a CI artifact or badge~~ done — artifact in ci.yml (`4e86443`; badge deliberately not wanted) | ~~Low~~ | ~~M~~ | ~~Quality~~ |
...
22 more rows resolved above: the tag ladder ran to v0.6.0 (CI + Releases +
proxy + pkg.go.dev all verified), GitHub polish completed (topics,
description, dependabot, templates, CODEOWNERS, social preview live), the
internal-name sweep closed as ACCEPT, and both consumers went public
(answering Q3). Still open: #2/#17 (docs/ fate — ROADMAP Q1, owner), #27/#28
(tracked-file and .gitignore reviews — never formally done), #29 (Issues vs
Discussions — never decided), #36 (license-badge consistency automation —
ROADMAP), #37 (go-public checklist as a skill — SKILLS repo), #40
(rename/repurpose evaluation — ROADMAP, gate passed 2026-09-11), #41 (profile
README links — unverified).
