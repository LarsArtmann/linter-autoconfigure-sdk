...
| b4 | Public-readiness of the repo surface | README exists and is decent (9 KB); MIT badge now truthful | README content never audited for external-consumer needs (GOEXPERIMENT=jsonv2 requirement lives only in AGENTS.md; install instructions unverified) | None | S |

## c) NOT STARTED

| # | Item | Why not started | Priority |
|---|------|-----------------|----------|
| c1 | CHANGELOG entry documenting relicensing + public flip | Not part of the explicit instruction; commit message carries the rationale but CHANGELOG.md is untouched | High |
| c2 | README §License fix: still links `LarsArtmann/template-LICENSE` repo instead of the now-existing `./LICENSE` | Discovered late (a1); not instructed | Medium |
...
| c5 | AGENTS.md not updated with new enduring facts: repo is public, MIT, no tags, no CI | Memory-maintenance rule applies; deferred to post-report | Medium |
...
| c7 | docs-health HARVEST of section (f) into TODO_LIST.md / ROADMAP.md | Report first (this file); harvest is the next step by design | High |
...
|---|------|--------|--------|----------|
| 1 | docs-health HARVEST: pull this table into TODO_LIST.md (1-15) and ROADMAP.md (16+) | High | S | Documentation |
...
| 5 | Add CHANGELOG entry: MIT relicensing + public flip (c1) | High | S | Documentation |
...
| 9 | Run `buildflow --fix --fail-on-findings` once to confirm the public tree is pipeline-green (not run since LICENSE change) | Medium | S | Quality |
| 10 | Fix README §License to link `./LICENSE` instead of the template repo (c2) | Medium | S | Documentation |
| 11 | Verify README documents the `GOEXPERIMENT=jsonv2` / direnv requirement for building from source (currently AGENTS.md-only; external consumers will hit it) | High | S | Documentation |
...
| 13 | Update AGENTS.md: public status, MIT, tag/CI state (c5) | Medium | S | Documentation |
...
| 22 | Review CONTRIBUTING.md (403 bytes) for external-contributor readiness: build steps, GOEXPERIMENT note, dprint, buildflow | Medium | M | Documentation |
...
| 25 | Verify no build artifacts ever entered history (e.g. `reports/coverage.out`) — `git log --all -- reports/` | Medium | S | Security |
| 26 | Confirm dprint/lint pipeline does not want to reformat LICENSE (plain text) — covered by item 9 | Low | S | Quality |
...
| 35 | State the v0 semver policy (breaking changes allowed until v1) in README or CONTRIBUTING | Medium | S | Documentation |
...
| 38 | Add markdown link/badge checker for README (dead template-LICENSE link is the first example of the class) | Low | S | Quality |
| 39 | Annotate this report once its items complete (docs-health ANNOTATE mode, non-destructive) | Low | S | Documentation |
...
| 42 | Consider `goreleaser`/release automation only if this ever ships binaries — for a library, tags + GitHub Releases suffice (note to prevent over-tooling) | Low | S | Roadmap |
...
| 45 | Sweep CHANGELOG.md formatting against Keep-a-Changelog since it now becomes a public-facing artifact | Low | S | Documentation |
...
*Point-in-time snapshot — will go stale. Harvest (f) into TODO_LIST.md/ROADMAP.md,
then annotate this file non-destructively as items complete.*
