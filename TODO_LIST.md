...
| T23 | Upload `docs/branding/social-preview.png` as the GitHub social preview (Settings UI only; no API exists for it)                                        | Low    | S      | flip report f30-32; asset exists at 1280x640                             |
| T24 | Re-check pkg.go.dev ~24h after 2026-09-10: confirm the post-T16 snapshot renders (`SaveJSON` two-value signature, updated `ExampleSaveJSON` output)   | Medium | S      | flip report f43; propagation lag is normal for new pseudo-versions       |
