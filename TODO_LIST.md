...
T15, T16, T17, T18, T19 — see CHANGELOG `[0.1.0]`.
...
| T24 | pkg.go.dev re-check: confirm the `v0.1.0` page renders (two-value `SaveJSON`, updated `ExampleSaveJSON`); proxy + `go get@v0.1.0` + consumer compile already verified 2026-09-10. pkg.go.dev lagged the proxy at release time (404 while proxy served the version) — propagation delay, retry later | Medium | S      | release session 2026-09-10; flip report f43                              |
