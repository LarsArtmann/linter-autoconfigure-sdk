...
| T24 | Post-push re-checks (~24h after 2026-09-10, once the auto-commit daemon has pushed): pkg.go.dev renders the post-T16 snapshot (`SaveJSON` two-value signature, updated `ExampleSaveJSON`); the CI badge URL resolves (404 until the workflow file lands on GitHub); re-run the throwaway-module consumer compile against the new pseudo-version | Medium | S      | flip report f43; propagation lag is normal for new pseudo-versions       |
